#ifndef EXT_DATA_MODEL_H
#define EXT_DATA_MODEL_H
...
#include "import_ext_data_model.h"
...
class ext_data_model {
public:
    tensors predict_ext(const tensors& inputs, internal::output_dict& output_cache)
    {
        internal::assertion(!is_stateful(),
                            "Prediction on stateful models is not const. Use predict_stateful instead.");
        return predict_ext_impl(inputs, output_cache);
    }
    ...
    {
        return ext_data_model_layer_->name_;
    }
    ...
    void reset_states()
    {
        ext_data_model_layer_->reset_states();
    }
...
private:
    ext_data_model(const std::shared_ptr<internal::ext_data_model_layer>& model_layer,
          const std::vector<tensor_shape_variable>& input_shapes,
          const std::vector<tensor_shape_variable>& output_shapes,
          const std::string& hash)
        : input_shapes_(input_shapes)
        , output_shapes_(output_shapes)
        , ext_data_model_layer_(model_layer)
        , hash_(hash)
    {
    }

    friend ext_data_model read_ext_data_model(std::istream&, bool,
                            const std::function<void(std::string)>&, float_type,
                            const internal::layer_creators&);

    tensors predict_ext_impl(const tensors& inputs, internal::output_dict& output_cache)
    {
        ...
        const auto outputs = ext_data_model_layer_->apply_ext(inputs, output_cache);
    ...
    tensors predict_impl(const tensors& inputs) const
    {
        ...
        const auto outputs = ext_data_model_layer_->apply(inputs);
    ...
    std::string hash_;
    std::shared_ptr<internal::ext_data_model_layer> ext_data_model_layer_;
};

// Load and construct an fdeep::model from an istream
// providing the exported json content.
// Throws an exception if a problem occurs.
inline ext_data_model read_ext_data_model(std::istream& model_file_stream,
                        bool verify = true,
                        ...
                        const internal::layer_creators& custom_layer_creators = internal::layer_creators())
{
    ...
    ext_data_model full_model(internal::create_ext_data_model_layer(
...
inline ext_data_model read_ext_data_model_from_string(const std::string& content,
                                    bool verify = true,
                                    const std::function<void(std::string)>& logger = cout_logger,
                                    float_type verify_epsilon = static_cast<float_type>(0.0001),
                                    const internal::layer_creators& custom_layer_creators = internal::layer_creators())
{
    ...
    return read_ext_data_model(content_stream, verify, logger, verify_epsilon,
                      custom_layer_creators);
}

// Load and construct an fdeep::model from file.
// Throws an exception if a problem occurs.
inline ext_data_model load_ext_data_model(const std::string& file_path,
                        bool verify = true,
                        const std::function<void(std::string)>& logger = cout_logger,
                        float_type verify_epsilon = static_cast<float_type>(0.0001),
                        const internal::layer_creators& custom_layer_creators = internal::layer_creators())
{
    ...
    const auto model = read_ext_data_model(in_stream, verify, logger, verify_epsilon,
                                  custom_layer_creators);
    if (logger) {
